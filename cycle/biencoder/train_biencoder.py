...
def evaluate(
    ...
    for step, batch in enumerate(iter_):
        ...
        with torch.no_grad():
            eval_loss, logits = reranker(context_input, candidate_input, step)  # blink原始代码
...
def main(params):
    ...
    train_data, train_tensor_data = data.process_mention_data(
        ...
        tokenizer,
        # 加入关系图3/6开始
        # params["structgraph_path"],
        # params["structgraph_size"],
        # params["featuregraph_path"],
        # params["featuregraph_size"],
        # 加入关系图3/6结束
        params["max_context_length"],
        params["max_cand_length"],
        # context_key=params["context_key"],  # blink原始代码1/1
    ...
    valid_samples = utils.read_dataset("validation", params["data_path"])
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            ...
            loss, _ = reranker(context_input, candidate_input, step)
            ...
            if (step + 1) % (params["print_interval"] * grad_acc_steps) == 0:
                logger.info("Step {} - epoch {} average loss: {}".format(step, epoch_idx, tr_loss / (params["print_interval"] * grad_acc_steps)))
                tr_loss = 0
