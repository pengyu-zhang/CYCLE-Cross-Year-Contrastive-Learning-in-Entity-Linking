...
def evaluate_gcl(embeds, ratio, idx_train, idx_val, idx_test, label, nb_classes, device, dataset, lr, wd
             , isTest=True):
    ...
    if isTest:
        # wandb.log({"Macro-F1_mean": np.mean(macro_f1s), "Micro-F1_mean": np.mean(micro_f1s), "AUC_mean": np.mean(auc_score_list)})
