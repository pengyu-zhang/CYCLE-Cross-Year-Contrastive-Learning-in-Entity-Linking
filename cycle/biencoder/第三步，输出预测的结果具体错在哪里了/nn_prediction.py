...
def get_topk_predictions(
    ...
    for step, batch in enumerate(iter_):
        ...
        context_input, _, srcs, label_ids = batch  # blink原始代码
        # context_input, _, srcs, relation_vec, label_ids = batch  # 加入关系图1/2
        ...
        scores = reranker.score_candidate(
            ...
            # relation_vec=relation_vec  # 加入关系图2/2
        )
        ...
        for i in range(context_input.size(0)):
            ...
            # 打印结果
            predicted_label = inds[0].item()
            actual_label = label_ids[i].item()
            results.append((oid, pointer != -1, "predicted_label: " + str(predicted_label), "actual_label: " + str(actual_label)))
