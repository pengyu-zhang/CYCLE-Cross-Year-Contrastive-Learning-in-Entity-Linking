...
class BiEncoderRanker(torch.nn.Module):
    ...
    def score_candidate(
        ...
        if random_negs:
            ...
            # 将 scores3 归一化到 -1 到 1 之间，然后乘以 X 来放大
            # scores3_normalized = torch.tanh(scores3) * 10
            scores2_normalized = torch.tanh(scores2) * weight  # 将 scores2 归一化到 -1 到 1 之间，然后乘以 X 来放大 new 13年最佳是20
            if scores1.shape != scores2_normalized.shape:
                scores = scores1  # 如果形状不一致，直接将scores设置为scores1
            else:
                scores = scores1 + scores2_normalized  # + scores3_normalized  # 如果形状一致，将scores1和scores2_normalized相加
            return scores
            # 方式1结束
    ...
    # def forward(self, context_input, cand_input, relation_vec, label_input=None):  # 加入关系图3/4
        ...
        optimiser1 = torch.optim.Adam(model1.parameters(), lr=args.lr, weight_decay=args.l2_coef)
        ...
        pkl_file_path = 'HeCo_' + own_str + '.pkl'
        if not os.path.exists(pkl_file_path):
            for epoch in range(args.nb_epochs):
                ...
                if loss1 < best:
                    ...
                    best_t = epoch
                    cnt_wait = 0
                    torch.save(model1.state_dict(), pkl_file_path)
                else:
                    cnt_wait += 1
            ...
            model1.load_state_dict(torch.load(pkl_file_path))
